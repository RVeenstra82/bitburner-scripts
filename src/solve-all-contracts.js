...
function caesarCipher(data, ns) {
    ...
    for(let i=0; i < startText.length; i++) {
        ...
        let code = startText.charCodeAt(i) + shift
        
        if (code1 > 90) {
            code1 = code1 - 26;
        }

        result1Array.push(String.fromCharCode(code1))
    }

    return result1Array.join("")
}
...
function vigenereCipher(data) {
    ...
    let str = data[0];    // number of vertices
    let keyword = data[1];    // adjacency data
    let key = ""

    // Generate the key to match the length of the str
    ...
    // Now cipher the text and write it to the response
    for (let i = 0; i < str.length; i++)
    {
        ...
        response+=String.fromCharCode(x);
    }
